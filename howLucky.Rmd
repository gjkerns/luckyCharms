...
Equipped with these data we can report things like the mean observed `Weight` was approximately 46.3g, the maximum number of a particular charm in any one bowl was 15 (Pink Hearts tied with Purple Horseshoes), and so forth.  We could spend all day computing statistics on this dataset to our Pink Heart's content, but at the moment we are primarily concerned with `Totcharms` and how it relates to `Bowl` and maybe `Weight` to a lesser extent.
...
This is a much noisier plot---as we might have guessed.  We have a nice range of weights, from a minimum under 30g up to a maximum near 70g.  Note that the lowest weights are usually associated with the last bowl of the box:  there isn't enough cereal remaining to make a complete 36g bowl.  And notice there is one bowl that was extraordinarily heavy. The explanation for where that could have come from isn't obvious, but if we dig a little deeper and plot `Weight` versus `Bowl` we may have a hint:
...
Now we see that the extra-heavy bowl was the last `Bowl = 12` of `Box = 1`.  The reason behind that particular data point has unfortunately been lost to the sands of time, but bearing in mind that it was the first box the team had ever finished, near the end it may have been difficult to judge how much cereal was left, and perhaps the rest was poured into that final twelfth bowl---I personally do the same thing all the time at breakfast as I near the end of a box of cereal.  Looking at the points on the graph, if that 12th bowl of almost 70g had been split into (say) two bowls of 40g and 30g, respectively, then there would have been two boxes that made it to 13 bowls instead of one box, and maybe the models below would have fit the observed data slightly better.  Alas!  We will never know. Such is the scientific enterprise.
...
We've got to do this one.   The code is slightly more involved than the previous examples and has been omitted for brevity, but you can check it all out [in this GitHub Gist](https://gist.github.com/gjkerns/8636439f9e6afdea1ba9d301b2b38947).  Let's get on with the plot:
...
I originally thought that either the whole thing would turn out to be a figment of my imagination or the effect would be too small to detect without LOTS AND LOTS of boxes of Lucky Charms.  Both alternatives turned out to be false.  The effect is real, and is big enough to detect with a couple handfuls of boxes.

## The Lucky Charms dropoff

The model `mod3` permits us to quickly arrive at some startling conclusions.  For example: how many charms do we estimate to be in the opening bowl of a box of Lucky Charms?  We saw earlier that the average `Weight` in this study was 46.3g.  When `Box = 1` the model estimates the average `Totcharms` to be
...
```
WOW.  25.3 charms on average.  This corresponds to just over a 52% reduction in charms from the first bowl to the eleventh.  No, it definitely wasn't my imagination, any hungry toddler should be able to detect this easily by taste alone.

Then the next question is: Why?  The standard explanation on physical grounds probably goes something like this: consider a box of Lucky Charms as a simple mixture of toasted oats and marshmallows. Over time, due to many external factors impacting the box such as jostling during transport, placement on the store shelf, transit to the home, and activity in and around the cupboard, there is a certain degree of movement throughout the box's contents, with the slightly less dense marshmallows migrating toward the top, and the more dense frosted toasty oats settling toward the bottom.  This is logical, anyway.  There are several open questions left that stand further investigation:

- Does the same pattern hold true for the individual charms? (A quick glance suggests "No".)
- Is the association truly linear, or would some amount of curvature better describe the relationshio?
- Are there strategies a person can use to decelerate the Lucky Charm dropoff?
...
Since the original experiment in Summer 2023, I've run the experiment a couple more times with different groups of students.  The first was in November 2023 with middle schoolers as part of [YSU MegaMath](https://megamath.ysu.edu/), and the second was in February 2024 with high schoolers as part of [YSU MathFest](https://mathfest.ysu.edu/).  I didn't give the MegaMath students very clear instructions and before I knew it all teams had removed the plastic bags of cereal from the boxes and were scooping from the middle of the bag lied flat on the tabletop. I don't blame the students; it much more expedient to scoop cereal from the middle of the bag, certainly, but unfortunately the approach effectively destroys any natural density-sorting of the marshmallows that may have have been present, the key underlying element we suspect is at play.  Moreover, nobody will convince me that their parents ever allowed them to eat their Lucky Charms that way.


By the time MathFest rolled around some months later I was better prepared and distributed a data collection sheet to the high school students ([which you can find here](https://gkerns.people.ysu.edu/LuckyCharmsDataCollectionSheet.docx)) with more detailed guidance and instructions.  The extra data collected are stored on Github in the `extraData` directory coming soon.

Moving forward, I would like to collect even more data and get ever-better estimates of the Lucky Charm dropoff, and search for procedures to uniformly distribute the marshmallows through the entirety of the cereal box.  If successful, the first bowl of the box might not be so magical, but on the other hand, maybe those final bowls won't feel like such a chore waiting for the next brand new box of Lucky Charms!


# Acknowledgements

This experiment and these results would not have been possible without the equal enthusiasm and diligent attention to detail of all four STAT 3743 students in Summer 2023: Brenna Brocker, Kate Coppola, Gavin Duwe, and Haziq Rabbani.  I would also like to thank the Department of Mathematics and Statistics at Youngstown State University for supporting this research and supporting additional data collection at YSU MegaMath and YSU MathFest.
