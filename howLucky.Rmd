...
Given these data we can report things like the mean observed `Weight` was approximately 46.3g, the maximum number of a particular charm in any one bowl was 15 (Pink Hearts was tied with Purple Horseshoes), and so forth.  We could spend all day computing statistics on this dataset to our Pink Heart's content, but at the moment, we are primarily interested in `Totcharms` and how that response variable is related to `Bowl` and maybe `Weight` to a lesser extent.
...
I originally thought that either the whole thing would turn out to be a figment of my imagination or the effect would be too small to detect without LOTS AND LOTS of boxes of Lucky Charms.  Both alternatives turned out to be false.  The effect is real, and it is big enough to detect with a mere handful of boxes.

The standard explanation for the effect on physical grounds would probably go along the lines of considering a box of Lucky Charms to be a simple mixture of cereal and marshmallows, with the notion that over time, due to many external factors affecting the box such as jostling during transport, placement on the store shelf, transit to the home, and activity in and out of the cupboard, that there should be a certain degree of settling in the box contents, with the less-dense marshmallows migrating toward the top, and the slightly more-dense toasted oats tending toward the bottom.  This is logical, anyway.  But there are several open questions yet to be investigated:
